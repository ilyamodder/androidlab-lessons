...
import ru.ilyamodder.weathersync.fragment.AddCityPopup;
import ru.ilyamodder.weathersync.fragment.CitiesList;
...
public class CitiesActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            getFragmentManager()
                    .beginTransaction()
                    .add(R.id.frame1, new CitiesList())
                    .commit();
        }
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.action_add) {
            new AddCityPopup().show(getFragmentManager(), AddCityPopup.class.getName());
            return true;
